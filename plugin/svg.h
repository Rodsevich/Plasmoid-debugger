 ...
 *
 **/
class CustomSvg : public QObject
{
    Q_OBJECT
    Q_ENUMS(ContentType)
    ...
    Q_PROPERTY(QString sorpi READ sorpi WRITE setSorpi NOTIFY sorpiChanged)
public:
    /**
     ...
     */
    explicit CustomSvg(QObject *parent = 0);
    ~CustomSvg();
...
public slots:
    void setSorpi(QString sorpi)
    {
	if (m_sorpi == sorpi)
	    return;

	m_sorpi = sorpi;
	emit sorpiChanged(sorpi);
    }
...
Q_SIGNALS:
    ...
    void customColorSchemeChanged(QVariantMap customColorScheme);

    void sorpiChanged(QString sorpi);
